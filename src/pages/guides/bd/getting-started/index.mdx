...
The Discord update on 27 September 2022 contained some major changes to fundamental characteristics of their production code.
A lot of named exports have had their names minified and module exports became read-only properties.
This changes the way internals can be used substantially.
...
You can find additional information on the [BetterDiscord documentation](https://docs.betterdiscord.app/) (still work in progress). 
...
BetterDiscord also provides the [`BdApi`](https://docs.betterdiscord.app/plugins/api/bdapi) global, which has a few very helpful things for plugins.
...
[`BdApi.React`](https://docs.betterdiscord.app/plugins/api/bdapi#react) and [`BdApi.ReactDOM`](https://docs.betterdiscord.app/plugins/api/bdapi#reactdom).
...
In `BdApi` we also find the [`BdApi.Patcher`](https://docs.betterdiscord.app/plugins/api/patcher), which is used to modify existing functions at runtime.
More details on patching will follow later on.

Lastly, [`BdApi.Webpack`](https://docs.betterdiscord.app/plugins/api/webpack) features a few utilities used to search through the webpack export cache.
...
[`BdApi.Webpack`](https://docs.betterdiscord.app/plugins/api/webpack) allows us to search through this export cache
in order to find variables, functions, objects or React components internally used by Discord.
[`Webpack.getModule(filter, options)`](https://docs.betterdiscord.app/plugins/api/webpack#getmodule) takes a filter function and searches through the exports of all currently cached modules.

[`Webpack.Filters`](https://docs.betterdiscord.app/plugins/api/filters) contains helpers to create commonly used search filter patterns.
The two most used ones are [`Filters.byProps(...props)`](https://docs.betterdiscord.app/plugins/api/filters#byprops)
and [`Filters.byDisplayName(name)`](https://docs.betterdiscord.app/plugins/api/filters#bydisplayname),
searching by properties of `module.exports` and a specific `displayName` on the export respectively.
...
Details in the following examples **are outdated**.
Discord's internals are undocumented and can change at any time.
...
The current props & hooks as well as the component type for the currently selected component can also be accessed in the console via the `$r` global.
...
This type of predicate is exactly what [`Filters.byProps(...props)`](https://docs.betterdiscord.app/plugins/api/filters#byprops) can be used for:
...
This is done using the already mentioned [`BdApi.Patcher`](https://docs.betterdiscord.app/plugins/api/patcher).
...
Remember: in our case we have a function component is wrapped inside a [React memo](https://reactjs.org/docs/react-api.html#reactmemo).
A memo stores the wrapped component in its `type` property.
We will simply leave the memo as it is and patch its `type` to hook into the contained function component.
...
or [my own plugin repo](https://github.com/Zerthox/BetterDiscord-Plugins/blob/master/dium/src/utils/react.ts#L6-L29) (BFS).
