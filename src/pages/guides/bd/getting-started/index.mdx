...
<Banner type="warn">Details in the following examples may be outed. Discord's internals are undocumented and can change at any time.</Banner>
...
The first way is the currently most common way to build search predicates for components.
...
The second way for formulating predicates is one that can only be applied in special cases.
...
As an example we will attempt to find the *UserStore* by reverse engineering the component responsible for rendering the account panel below the channel list.
Searching through the component tree for a component which does not receive the data as props from a parent brings us to a component called `Tee`.
