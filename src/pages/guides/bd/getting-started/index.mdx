...
Sometimes taking a shot in the dark with generic searches can help finding the right module as well.
For example, we could search for all modules exporting something with `"friend"` in the key:
...
If we know we want to look for a store instance, we could instead search through the keys of the prototype instead:
...
or [my own plugin repo](https://github.com/Zerthox/BetterDiscord-Plugins/blob/master/discordium/utils/react.ts#L6-L29) (BFS).
